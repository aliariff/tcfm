...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null, $max_thumb, $path) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/assets/img/'.$path,
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                $time = now();
                $nama_file = do_hash($time . $upload['name'][$index], 'sha1');
                $files[] = $this->handle_file_upload(
                    $upload['tmp_name'][$index],
                    $file_name ? $file_name : $nama_file,
