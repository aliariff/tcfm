...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 05, 2014 at 06:19 PM
...
--
CREATE DATABASE IF NOT EXISTS `webmmowe_tcfm` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `webmmowe_tcfm`;
...
-- Procedures
--
CREATE PROCEDURE `sp_baca_tawaran`(p_username varchar (45))
BEGIN
	update `penawaran` set `terbaca` = 1 where `penawar`=p_username and `jawaban`!=99;
END$$

CREATE PROCEDURE `sp_batalkan_tawaran`(p_username varchar (45), p_id int (11))
start_label: BEGIN
...
CREATE PROCEDURE `sp_beliPemain`(
...
CREATE PROCEDURE `sp_best_team_lineup`(p_username varchar (45))
BEGIN
...
CREATE PROCEDURE `sp_cek_biaya_perawatan_stadion`(p_username varchar(45))
start_label: BEGIN
	declare biaya int;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND
	BEGIN
...
CREATE PROCEDURE `sp_cek_latihan`(p_username varchar (45))
start_label: BEGIN
...
CREATE PROCEDURE `sp_clear_lineup`(p_username varchar (45))
BEGIN
	update `user_pemain` set `aktif` = 0 where `user_username` = p_username;
	select 0 as kode, 'Clear Lineup Berhasil' as pesan;
END$$

CREATE PROCEDURE `sp_daftar_formasi`()
BEGIN
	select * from `formasi`;
    END$$

CREATE PROCEDURE `sp_daftar_latihan`()
BEGIN
  select 
    * 
  from
...
CREATE PROCEDURE `sp_daftar_lineup`(p_username varchar(45))
BEGIN
	select *
	from `user_pemain`, `pemain`
	where `user_username` = p_username
...
CREATE PROCEDURE `sp_daftar_pemain`()
BEGIN
	select
	`id_pemain`,
	`nama_pemain`,
...
CREATE PROCEDURE `sp_daftar_pemain_ai`(p_id_tim_ai int (11))
BEGIN
	SELECT *
...
CREATE PROCEDURE `sp_daftar_pemain_user`(p_username varchar (45))
BEGIN
  select 
    `id_user_pemain`,
    `id_pemain`,
...
CREATE PROCEDURE `sp_daftar_penawaran_keluar`(p_username varchar(45))
BEGIN
	SELECT
	`id_penawaran`,
	`harga_penawaran`,
...
CREATE PROCEDURE `sp_daftar_penawaran_masuk`(p_username varchar (45))
BEGIN
	select
	`id_penawaran`,
	`harga_penawaran`,
...
CREATE PROCEDURE `sp_daftar_stadion`()
BEGIN
	select `id_stadion`,`biaya_perawatan`,`harga_stadion`,`pendapatan_persen`,`gambar_stadion`
	from `stadion`;
    END$$

CREATE PROCEDURE `sp_daftar_tim_liga`(p_id_liga int (11))
BEGIN
  select 
    `id_tim_ai`,
    `nama_tim_ai`,
...
CREATE PROCEDURE `sp_detail_pemain`(p_id_user_pemain int(11))
BEGIN
select *
...
CREATE PROCEDURE `sp_detail_pemain_id`(p_id int)
BEGIN
	select * from `pemain`
	where `id_pemain` = p_id;
END$$

CREATE PROCEDURE `sp_downgrade_stadion`(p_username VARCHAR(45))
start_label: BEGIN
...
CREATE PROCEDURE `sp_getAllLimited`()
BEGIN
	select * from pemain
...
CREATE PROCEDURE `sp_getAllSetting`()
BEGIN
	select
	`server_waktu_mulai`,
	`server_durasi_tu`,
...
CREATE PROCEDURE `sp_getAllTimAi`(p_username varchar (45))
BEGIN
	select `id_tim_ai`,
	`nama_tim_ai`,
	`kekompakan_tim_ai`,
...
CREATE PROCEDURE `sp_getAllUser_Offset`(off int, per_page int)
BEGIN
	SELECT
		`username`,
		`nama_tim`,
...
CREATE PROCEDURE `sp_getFormasiAI`(p_id int)
BEGIN
...
CREATE PROCEDURE `sp_getLimited`()
BEGIN
	select * from `pemain`
	where `flag_limited` = 1
	and `flag_tersedia` != 0
...
CREATE PROCEDURE `sp_getUserInfo`(p_username varchar (45))
BEGIN
  select 
    `nama_tim`,
    `nama_stadion`,
...
CREATE PROCEDURE `sp_release_pemain`(p_username varchar (45), p_id int)
start_label: BEGIN
...
CREATE PROCEDURE `sp_setKekompakan`(
  p_username VARCHAR (45),
...
CREATE PROCEDURE `sp_tambahBalen`(
  p_username varchar (45),
  kode varchar (45)
)
BEGIN
...
CREATE PROCEDURE `sp_tambah_formasi`(p_nama_formasi varchar(45))
BEGIN
...
CREATE PROCEDURE `sp_tambah_latihan`(
...
CREATE PROCEDURE `sp_tambah_pemain`(
...
CREATE PROCEDURE `sp_tanding_ai`(att varchar (45), def int (11))
start_label:BEGIN
...
END;
	...
	IF (hasil = 2) THEN #menang
		...
		if (ctr = ctr_max) then
			...
			#naikkan liga
			if ((select `liga_id_liga` from `user` where `username`=att) != 3) then
...
CREATE PROCEDURE `sp_tanding_pemain`(att varchar(45), def varchar(45))
start_label:BEGIN
DECLARE rating_att, rating_def, kekompakan_att, kekompakan_def, total_att, total_def, rumus_formasi, nk, hasil, exp_gain, kekompakan_loss, uang_gain double;
DECLARE EXIT HANDLER FOR SQLEXCEPTION
BEGIN
...
CREATE PROCEDURE `sp_tawar_pemain`(
  p_harga int (10),
...
CREATE PROCEDURE `sp_terima_tawaran`(p_username varchar(45), p_id int(11))
start_label: BEGIN
...
CREATE PROCEDURE `sp_tolak_tawaran`(p_username VARCHAR(45), p_id INT(11))
start_label: BEGIn
	DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND
	BEGIN
		ROLLBACK;
...
CREATE PROCEDURE `sp_ubahSetting`(
...
CREATE PROCEDURE `sp_ubah_password`(
  p_username varchar (45),
  p_lama varchar (45),
  p_baru varchar (45),
  p_rebaru varchar (45)
...
CREATE PROCEDURE `sp_ubah_profil`(
  p_username varchar (45),
  p_nama_tim varchar (45),
  p_nama_stadion varchar (45),
  p_email varchar (45),
...
CREATE PROCEDURE `sp_update_formasi`(p_username varchar(45), p_id_formasi int(10))
BEGIN
DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND
BEGIN
	ROLLBACK;
...
CREATE PROCEDURE `sp_update_lineup`(
	p_username VARCHAR(45), 
...
CREATE PROCEDURE `sp_update_lineup_limited`(p_id int, pos int)
BEGIN
DECLARE id_up INT(11);
   START TRANSACTION;
...
CREATE PROCEDURE `sp_upgrade_stadion`(p_username varchar(45))
start_label: BEGIN
DECLARE harga INT(10);
DECLARE EXIT HANDLER FOR SQLEXCEPTION, SQLWARNING, NOT FOUND
	BEGIN
...
CREATE PROCEDURE `trial`(p_userid varchar(45))
BEGIN
...
-- Functions
--
CREATE FUNCTION `fn_ap`(p_username varchar (45)) RETURNS tinyint(3)
BEGIN
  RETURN 
  (SELECT 
    IF (
...
CREATE FUNCTION `fn_count_all_user`() RETURNS int(11)
BEGIN
  return (
...
CREATE FUNCTION `fn_exp`(p_username varchar (45)) RETURNS int(11)
BEGIN
  return (
...
CREATE FUNCTION `fn_hitungLineup`(p_username varchar (45)) RETURNS int(11)
BEGIN
  return (
...
CREATE FUNCTION `fn_jumlah_pemain_tim`(p_username varchar (45)) RETURNS int(11)
BEGIN
  return (
...
CREATE FUNCTION `fn_kekompakan`(p_username varchar(45)) RETURNS tinyint(3)
BEGIN
	return(
...
CREATE FUNCTION `fn_level`(p_username varchar (45)) RETURNS int(11)
BEGIN
  declare expe double ;
  SELECT 
    `exp` INTO expe 
...
CREATE FUNCTION `fn_nama_formasi`(p_id_formasi int (11)) RETURNS varchar(45) CHARSET utf8
BEGIN
  return 
  (select 
    `nama_formasi` 
...
CREATE FUNCTION `fn_nama_latihan`(p_id_latihan int (11)) RETURNS varchar(45) CHARSET utf8
BEGIN
  return 
  (select 
    `nama_latihan` 
...
CREATE FUNCTION `fn_nama_liga`(p_id_liga int (11)) RETURNS varchar(45) CHARSET utf8
BEGIN
  return 
  (select 
    `nama_liga` 
...
CREATE FUNCTION `fn_persentase_stadion`(p_username varchar (45)) RETURNS int(11)
BEGIN
  return (
...
CREATE FUNCTION `fn_rating_pemain`(p_pos CHAR(1), p_att INT(10), p_def INT(10), p_speed INT(10), p_stamina INT(10)) RETURNS int(10)
BEGIN
	IF(p_pos = 'F') THEN
	RETURN(
...
CREATE FUNCTION `fn_rating_tim`(p_userid VARCHAR(45)) RETURNS int(10)
BEGIN
    DECLARE rating, def_limit, mid_limit, att_limit, minus_rate, counter, pl, temp_rate INT(10);
    DECLARE nama_form VARCHAR(45);  
    DECLARE temp_posisi CHAR(1); 
...
CREATE FUNCTION `fn_rating_tim_ai`(p_id int) RETURNS int(11)
BEGIN
...
CREATE FUNCTION `fn_rumus_formasi`(att varchar (45), def varchar (45)) RETURNS int(11)
BEGIN
return (
...
CREATE FUNCTION `fn_rumus_formasi_ai`(att varchar (45), def int) RETURNS int(11)
BEGIN
  RETURN (
	SELECT `nilai_rumus` FROM `rumus_formasi`
	WHERE `formasi_id_formasi` = (
...
CREATE FUNCTION `fn_server_time_TU`() RETURNS int(11)
BEGIN
  RETURN 
  (SELECT 
    FLOOR(
...
CREATE FUNCTION `fn_statusProteksi`(p_att varchar (45), p_def varchar (45)) RETURNS tinyint(1)
BEGIN
  RETURN 
  (SELECT 
    (
...
CREATE FUNCTION `fn_uang`(p_username varchar (45)) RETURNS int(10)
BEGIN
  return 
  (select 
    `uang` 
